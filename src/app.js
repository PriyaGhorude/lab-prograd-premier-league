...
{
  var managerInfo = [];
  //managerInfo =[managerName,managerAge,currentTeam,trophiesWon];
  managerInfo.push(managerName,managerAge,currentTeam,trophiesWon);
  return managerInfo;
}
// if(manager.length == 4)
// {
...
function filterByDebut(year)
{
  ...
  //
  players.filter(item =>{
    return item.debut == year ? playerArray.push(item)  : playerArray;
  });
  return playerArray;
}
...
{ 
  ...
  {
      players.filter(item =>{
      item.position == position ? positionArray.push(item) : positionArray;
    });
  }
  return positionArray;
